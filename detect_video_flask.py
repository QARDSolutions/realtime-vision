...
def get_od(frame):
    ...
    api_bbox = {'coordinates':[], 'scores':[], 'classes':[], 'valid_detections':[]}
    #print(pred_bbox)
    #print('scores: ', scores.numpy())
    #print('classes: ', classes.numpy()[0])
    print('valid_detections: ', valid_detections.numpy()[0])
    pred_scores = list(scores.numpy()[0])
    pred_classes = classes.numpy()[0]
    pred_valid_detections = valid_detections.numpy()[0]
    #print('pred_scores: ', pred_scores[0])
    for i in range(pred_valid_detections):
        if np.sum(bboxes[i])!=0:
            api_bbox['coordinates'].append([str(val) for val in bboxes[i]])
            api_bbox['scores'].append(str(pred_scores[i])) 
            class_name = allowed_classes[int(pred_classes[i])]
            api_bbox['classes'].append(class_name) 
    api_bbox['valid_detections'] = str(pred_valid_detections)
    # read in all class names from config
    #print(api_bbox)
    api_resp['bboxes'] = api_bbox
    ...
    if FLAGS_COUNT:
        ...
        for key, value in counted_classes.items():
            api_resp['detections'][key] = str(value)
...
def main():
    ...
    if cap.isOpened():
        ...
        #encoding frame for api
        _, img_encoded = cv2.imencode('.png', frame)
        api_resp['frame'] = base64.b64encode(img_encoded).decode()#frame.encode('base64')
        #img = decodeBase642Img(api_resp['frame'])
        print(info)
        #print(api_resp)
