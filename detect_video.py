...
def get_od(frame):
    api_resp = {'userId':'605752d0523db426502997d7', 'deviceId':'606b101d3019563694f99c3d', 'eventType':'odnet',
                'person':'0', 'bicycle':'0', 'car':'0', 'motorbike':'0', 'truck':'0', 'cat':'0', 'dog':'0', 
                'backpack':'0', 'handbag':'0', 'suitcase':'0', 'knife':'0'}
...
def sendResponseAPI(api_resp, headers):
    payload = json.dumps(api_resp)
    conn.request("POST", "/events-log/", payload, headers)
...
def main():
    ...
    while (cap.isOpened()):
        ...
        if frame_buffer.empty() != True:
            ...
            #print(api_resp)
            #sendResponseAPI(api_resp, headers)
            #processing end, info start
            fps = 1.0 / (time.time() - start_time)
            fps_str = 'FPS: %.2f  |  ' % fps
            #PrintInline(info)
            overprint(fps_str + info, length)
            length = len(fps_str + info)
